    ...
    <v-row>
      <v-col>
        ...
        >
          ...
          <v-card-text>
            <p>
              Pagamento no valor de <strong>R$ {{ filterIntToReal(valorPrincipal) }}</strong> para <strong>{{ nomeUnidade }}</strong> pelo servi&ccedil;o <strong>{{ nomeServico }} ({{ codigoServico }})</strong>.
            </p>
            <p>
              O pagamento <template v-if="competencia">
                referente ao m&ecirc;s <strong>{{ dayjs(competencia, 'YYYY-MM').format('MM/YYYY') }}</strong>&nbsp;
              </template>est&aacute; em nome de <strong>{{ nomeContribuinte }}<template v-if="cnpjCpf">&nbsp;({{ filterCnpjCpf(cnpjCpf) }})</template></strong>.
            </p>
            <p v-if="referencia">
